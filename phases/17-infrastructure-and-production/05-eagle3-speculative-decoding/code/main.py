"""玩具版推测解码分析器，使用 Python stdlib。

计算 EAGLE-3 风格推测解码在一系列 (alpha, K, verify_overhead, concurrency)
参数点上的预期加速比和盈亏平衡 alpha。用于教学：数值体现趋势，而非绝对延迟。
"""
...
class SpecPoint:
    alpha: float      # 接受率（0..1）
    k: int            # draft 长度
    verify_overhead: float  # 每次 target forward 的额外成本比例
    concurrency: int  # decode 时的批次大小


def expected_speedup(p: SpecPoint) -> float:
    """普通 decode：每次 target forward 生成 1 个 token。
    (alpha, K) 下的推测 decode：每次 target forward 预期生成 1 + K*alpha 个 token，
    但每次 target forward 相对普通 decode 的成本为 (1 + verify_overhead)。
    并发会增加 verify_overhead（更多序列共享验证成本）。
    """
...
def simulate_tail(p: SpecPoint, n_tokens: int = 1000, seed: int = 3) -> tuple[float, float]:
    """模拟逐 token 延迟分布。
    普通 decode：每个 token 的延迟近似恒定（加少量抖动）。
    推测 decode：有效 token 成批到达；draft 被拒绝时要付出两次 target pass 的成本。
    返回 (mean_ms, p99_ms)。
    """
    ...
    reroll_ms = base_target_ms  # draft 提前拒绝时的第二次 pass

    latencies: list[float] = []
    tokens_emitted = 0
    while tokens_emitted < n_tokens:
        # draft K 个 token，然后验证
        accepted = 0
        for _ in range(p.k):
        ...
        # 输出 token 数：accepted + 1（末尾经过验证的那个）
...
def main() -> None:
    ...
    print(f"配置：K={k}，基础 verify_overhead={base_overhead}")
    print()
    ...
    for concurrency in [32, 128, 256]:
        ...
        print(f"  --- 并发数 {concurrency} ---  普通 decode P99 = {plain_p99:.2f} ms")
    ...
    print("关键发现")
    print("-" * 80)
    print("  盈亏平衡 alpha 会随并发数上升。并发数为 32 时，")
    print("  只要高于约 0.1 就有收益；并发数为 256 时，门槛约为 0.4。低于该值时，")
    print("  即使预期加速公式为正，P99 长尾也会恶化。")
    print("  发布前请在真实流量上测量 alpha。")
