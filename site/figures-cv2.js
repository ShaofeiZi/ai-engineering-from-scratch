/* figures-cv2.js — Phase 4(计算机视觉)的动画 SVG 课程插图。
   在 lesson-figures.js 之后加载,并通过 window.LF 注册组件。
   每个插图都是一个自运行的 SMIL 动画,演示一个 CV 概念:没有 JS
   定时器,没有计算循环。纯 ES5,无依赖,通过 CSS 变量设置主题。
...
(function () {
  ...
  // ── object-detection-nms(06):候选框弹出,NMS 剪除重叠 ──
  ...
  // ── segmentation-flood(07):编码器→瓶颈→解码器,区域洪水填充 ──
  function segmentationFlood(host) {
    ...
    // 一个 6x6 区域图:为每个单元格分配类别 id,按波次洪水填充
  ...
  // ── gan-minimax(09):G 将噪声变为图像,D 的判决来回摆动 ───
  function ganMinimax(host) {
    ...
    // 伪造图像:4x4 网格,在循环过程中从噪声锐化为连贯图案
    var gx = 290, gy = 40, cs = 18, r, c;
    for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) {
      var target = ((r < 2) === (c < 2)) ? 0.85 : 0.18; // 一个 2x2 "人脸"块图案
  ...
  // ── diffusion-denoise(10):噪声网格逐步回到干净图像 ────────
  function diffusionDenoise(host) {
    ...
    // 一个 6x6 网格:每个单元格从随机噪声不透明度动画到目标图像
  ...
  // ── clip-contrastive(18):NxN 相似度矩阵,对角线亮起 ────────
  ...
  // ── depth-rays(26):RGB 网格 → 深度梯度,扫描扫掠按距离着色 ─
  function depthSweep(host) {
    ...
    for (r = 0; r < N; r++) for (c = 0; c < N; c++) {
      ...
      var op = (0.25 + 0.6 * (1 - d)).toFixed(3); // 近 = 更亮
