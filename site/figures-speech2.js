/* figures-speech2.js — Phase 6(语音与音频)的动画课程插图。
   在 lesson-figures.js 之后加载,并通过 window.LF 注册组件。
   纯 ES5,无依赖,通过 CSS 变量设置主题。SMIL 动画 SVG,无 JS 循环。
...
(function () {
  ...
  // ── sp-asr-attention:解码器对编码器音频帧做交叉注意 ──────
  function asrAttention(host) {
    var svg = svgEl('svg', { viewBox: '0 0 520 230' });
    // 编码器音频帧沿顶部排列,解码器 token 从下方发出,
    // 每个新 token 点亮一条柔和的交叉注意权重对角带。
  ...
  // ── sp-eer-crossover:FAR 与 FRR 曲线在 EER 阈值处交叉 ─────
  function eerCrossover(host) {
    ...
    // FRR 随阈值上升,FAR 下降;二者在 t=0.5 附近交叉 -> EER。
  ...
  // ── sp-tts-stack:文本 -> mel 频谱图 -> 波形,三个阶段 ────────
  ...
  // ── sp-codec-tokens:RVQ 码本堆栈馈送自回归解码器 ────
  ...
  // ── sp-vad-cascade:波形由语音/静音门控并带挂起定时器 ───
  ...
  // ── sp-voice-factorize:拆分内容 + 说话人,交换说话人,重组 ───
