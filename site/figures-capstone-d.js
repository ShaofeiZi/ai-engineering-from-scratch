/*图像-capstone-d.js - - 动画课程图像为第19阶段的51-87范围的顶石项目 (查询重写 / HyDE,经典指标,困难度 +校准, ZeRO 分片,管道平行, jailbreak分类,提示注射检测器,宪法规则引擎). 课程后载-figures.js,通过窗口注册.LF. SMIL 运动只 - 没有JS循环,没有rAF.ES5,没有 deps,通过CSS进行主题. */
(function () {
  'use strict';
  var LF = window.LF;
  if (!LF) { return; }
  ...
  // ── cd-hyde-vector (67):查询向量错过,假设的文档落地目标 ─
  ...
  // ── cd-blue-overlap (71):与参考相匹配的候选 n-gram ─────
  ...
  function cdReliability(host) {
    ...
    var acc = [0.08, 0.2, 0.34, 0.5, 0.62]; // 过度自信
    var i;
  ...
  // ── cd-zero-shard (78):优化状态分为数列,散射然后收集 ─
  ...
  function cdPipeline(host) {
    ...
    // 横向流量:微分量 m 在时间 t = m + i 上占据了 i 阶段
    var period = 4.4;
    for (j = 0; j < M; j++) {
      for (i = 0; i < N; i++) {
        var slot = j + i;
  ...
  // ── cd-output-router (85):三个输出分类器为政策路由器提供了输出
  ...
  // ── cd-constitution-loop (86)：草稿、规则、修复、修订与复检闭环 ──
