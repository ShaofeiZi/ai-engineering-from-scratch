...
![FID、CLIP 与偏好：三个维度，各有不同失效模式](../../../../../../phases/08-generative-ai/14-evaluation-fid-clip-score/assets/evaluation.svg)
...
失效模式：
- **小样本 N 下存在偏差。** 均值与协方差的有限样本估计含有噪声，会使 FID 失真。即使两个样本来自同一分布，通常也会得到正的分数（即演示中展示的向上偏差）；而误差的方向和幅度取决于具体分布与特征空间。务必使用 N ≥ 10,000。
