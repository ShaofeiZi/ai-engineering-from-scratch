...
- 区分 SAM（只接受视觉提示）、Grounded SAM / SAM 2（检测器 + SAM）和 SAM 3（通过“可提示概念分割”原生接受文本提示）
...
SAM 3（Meta，2025 年 11 月，ICLR 2026）把这条级联合并起来。它接受短名词短语或图像示例作为提示，并在一次前向传播中返回所有匹配掩码与实例 ID。这就是**可提示概念分割（PCS）**。再结合 2026 年 3 月的 Object Multiplex 更新（SAM 3.1），它可以高效追踪视频中同一概念的多个实例。
