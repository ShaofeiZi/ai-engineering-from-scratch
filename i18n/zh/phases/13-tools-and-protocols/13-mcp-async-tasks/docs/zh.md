...
因此，网关必须把请求协调器和任务路由保存在不同的表中。请求表可以在响应完成时消失，任务路由则必须存活到终态以及保留期结束。[第 29 课：MCP 可靠性、取消与流量控制](../../29-mcp-reliability-cancellation-and-flow-control/docs/en.md)为两条路径建立竞态、超时、幂等性、背压与重试规则。
