...
[第 16 课：使用 OAuth 2.1 保护 MCP](../../16-mcp-security-oauth-2-1/docs/zh.md)负责授权码状态机、PKCE、受保护资源发现、resource indicator 和 scope 决策。本课不会定义第二套 OAuth 流程。它从这些契约已经存在的地方开始，讨论已部署的资源服务器如何在密钥轮换、不透明令牌验证、吊销、依赖故障、发布与事件响应期间继续执行这些契约。
