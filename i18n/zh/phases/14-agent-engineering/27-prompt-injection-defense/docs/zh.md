...
边界很重要：该模式将模型提出的调用、工具参数和检索内容视为不可信，而宿主进程和已注册的工具实现仍属于可信代码。Python 对象的私有性无法隔离恶意的进程内工具。应在独立进程或服务中运行不可信工具，并把一次性授权的消费状态放入外部原子存储，使其能够跨工作进程和重启保持有效。

Unicode 安全同样存在版本边界。本示例会对运行时 Unicode 数据库仍标记为未分配的码点采取失败关闭策略（明确列出的默认可忽略控制字符除外）。这样可以防止较新 Unicode 版本引入的兼容字符绕过旧运行时的规范化。生产服务应端到端固定并测试同一个 Unicode 数据版本。
