# Prompt Injection 与 Prompt-Validator-Executor 防御
...
### Prompt-Validator-Executor（PVE）验证模式
...
边界很重要：该模式将模型提出的调用、工具参数和检索内容视为不可信，而宿主进程和已注册的工具实现仍属于可信代码。Python 对象的私有性无法隔离恶意的进程内工具。应在独立进程或服务中运行不可信工具，并把一次性授权的消费状态放入外部原子存储，使其能够跨工作进程和重启保持有效。每项授权都要绑定到最新的可信用户轮次：如果后续用户消息没有携带匹配授权，就应撤销较早的请求，而不能让 validator 向前搜索并复用已经过期的许可。

Unicode 安全同样存在版本边界。本示例会对运行时 Unicode 数据库仍标记为未分配的码点采取失败关闭策略（明确列出的默认可忽略控制字符除外）。它还只在检查已知危险指令短语时应用一套范围有限、可审计的易混淆字符骨架，使跨文字系统的相似字符无法隐藏危险标记，同时不影响普通的单一文字系统文本。生产服务应端到端固定并测试同一个 Unicode 数据版本。
...
`code/main.py` 实现了 Prompt-Validator-Executor 验证模式：
...
- **OpenAI Agents SDK guardrails**（Lesson 16）：内置了采用 Prompt-Validator-Executor 形态的模式。
...
- **Custom Prompt-Validator-Executor layer**：为你的业务域定制 validator 模型，用来识别特定注入模式。

## 交付成果

`outputs/skill-injection-defense.md` 为任何代理运行时提供一层 Prompt-Validator-Executor 验证层脚手架，以及配套的内容捕获纪律。
...
5. 做统计：在正常流量下，Prompt-Validator-Executor validator 会拒绝多少次？目标应该是在合法调用上接近零误杀。
...
- [OpenAI Agents SDK docs](https://openai.github.io/openai-agents-python/) — guardrails 可作为一种 Prompt-Validator-Executor 风格的控制措施
